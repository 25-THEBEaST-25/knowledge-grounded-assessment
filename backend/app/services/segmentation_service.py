...
PaddleOCR or an LLM. Detects question markers such as::
    ...
    1. / 1) / (1) / 1:      (only when the number is at the start of a line)
    Q1(a) / Q1a / 1(a)     -> sub-question, e.g. "Q1a"
...
# sub-question: "(a)" / "(iv)" anywhere after the number, or a bare letter
# glued to the number and followed by a separator, e.g. "Q1a." / "1b)"
_SUB = r"(?:\s*\((?P<sub>[a-hA-H]|[ivx]{1,4})\)|(?P<sub2>[a-hA-H])(?=\s*[\.\):\-]))?"
_SEP = r"\s*[\.\):\-–—]?\s*"

_PATTERNS = [
    # Q1, Q.1, Q 1, Q1), Q1:, Q1(a), Question 1, Ques 1, Que 1
    re.compile(rf"^\s*(?:Q|Que|Ques|Question|Qn|Qu)\s*\.?\s*{_NUM}{_SUB}{_SEP}(?P<rest>.*)$", re.I),
    # Ans 1, Answer 1, Ans. 1, A1 (A1 requires a separator to avoid words)
    re.compile(rf"^\s*(?:Ans|Answer|Ans\.)\s*\.?\s*{_NUM}{_SUB}{_SEP}(?P<rest>.*)$", re.I),
    re.compile(rf"^\s*A\s*\.?\s*{_NUM}{_SUB}\s*[\.\):\-]\s*(?P<rest>.*)$"),
    # 1. / 1) / (1) / 1: / 1(a)  at start of line, must have a separator
    re.compile(rf"^\s*\(?{_NUM}{_SUB}\)?\s*[\.\):\-]\s+(?P<rest>.*)$"),
    # 1(a) / 1 (b) at start of line: the parenthesised sub-part is the separator
    re.compile(rf"^\s*\(?{_NUM}\)?\s*\((?P<sub>[a-hA-H]|[ivx]{{1,4}})\)\s*[\.\):\-]?\s+(?P<rest>.*)$"),
    # OCR mis-read of "Q.1" as "0.1" / "0 1" at line start
    re.compile(rf"^\s*0\s*[\.\s]\s*(?P<num>[1-9][0-9]?){_SUB}{_SEP}(?P<rest>.*)$"),
]
...
def normalise_question_id(num: str, sub: Optional[str] = None) -> str:
    num = num.translate(_DIGIT_FIX)
...
def match_marker(line: str):
    ...
    for pattern in _PATTERNS:
        ...
        if not fixed.isdigit() or int(fixed) == 0 or int(fixed) > 200:
            continue
        groups = m.groupdict()
        sub = groups.get("sub") or groups.get("sub2")
        return normalise_question_id(num, sub), m.group("rest").strip()
    return None
...
def segment_answers(
    ...
    - Text before the first marker is kept as ``unassigned_preamble`` and also
      prepended to the first segment (a heading is harmless to the evaluator).
    - If no markers are found the whole text becomes one segment. Its id is
      the first ``expected_question_ids`` entry if given, else ``Q1``.
    ...
    for idx, raw in enumerate(lines):
        ...
        if marker:
            ...
            current = Segment(question_id=qid, text="", start_line=idx, end_line=idx, marker_line=raw.strip())
    ...
    if not result.segments:
        if result.unassigned_preamble:
            qid = expected_question_ids[0] if expected_question_ids else "Q1"
    ...
    if expected_question_ids:
        ...
        for qid in expected_question_ids:
            if qid.lower() not in present:
                result.segments.append(
                    Segment(question_id=qid, text="", start_line=-1, end_line=-1, detected=False)
                )
        order = {q.lower(): i for i, q in enumerate(expected_question_ids)}
